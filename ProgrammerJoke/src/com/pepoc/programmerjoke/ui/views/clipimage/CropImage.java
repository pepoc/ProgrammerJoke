package com.pepoc.programmerjoke.ui.views.clipimage;
...
public class CropImage {
	...
	public String saveToLocal(Bitmap bm) {
		File file = new File(Util.getBaseFilePath(), "temp.jpg");
		try {
	...
	class BackgroundJob implements Runnable {
		...
		public void run() {
				...
				public void run() {
					try {
						mHandler.sendMessage(mHandler
								.obtainMessage(ClipImageActivity.SHOW_PROGRESS));
			...
			try {
				...
				mHandler.sendMessage(mHandler
						.obtainMessage(ClipImageActivity.REMOVE_PROGRESS));
				// mHandler.post(new Runnable()
				// {
				// public void run()
				// {
